...
import { numDaysInMonth, TODAY } from '../../../lib/date';
...
class SimpleDatePickerContainer extends React.PureComponent<Props, State> {
  ...
  handleChange = (e: Event, { value, placeholder }: any) => {
  ...
  handleDateChange = () => {
    ...
    const actualDay: number = day === DAY_PLACEHOLDER_VALUE ? 1 : (day as number);
    const actualMonth = month === MONTH_PLACEHOLDER_VALUE ? 0 : getMonthIndex(month!) - 1;

    const date =
      year === YEAR_PLACEHOLDER_VALUE ? null : new Date(year as number, actualMonth, actualDay);

    if (date && handleDateSelection) {
  ...
  render() {
    ...
    const daySelection = getDaySelection(month as number, year as number);
    const monthSelection = getMonthSelection(year as number);
...
const getDaySelection = (month?: DateSelection, year?: DateSelection): any[] => {
  ...
  return [
    ...
    ...range(1, numDaysInMonth(monthIndex, year as number) + 1).map(day => ({
      text: day,
      value: day,
    })),
  ];
...
const yearOptions = [
  ...
  ...range(TODAY.getFullYear(), 1995, -1).map(year => ({
    text: year,
    value: year,
  })),
];
